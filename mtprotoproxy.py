...
# 在CryptoHelper类中增加额外的密钥派生函数
def derive_key(self, key, salt):
    # 使用 PBKDF2 进行密钥派生
...
def init_config():
    ...
    conf_dict.setdefault("MAX_CONNECTIONS", 1000) 
    conf_dict.setdefault("MAX_CONNECTIONS_PER_IP", 20)
    conf_dict.setdefault("MIN_PADDING_SIZE", 16)
    conf_dict.setdefault("MAX_PADDING_SIZE", 32)
    conf_dict.setdefault("HANDSHAKE_TIMEOUT", 10)
...
def update_handshake_cache():
    global used_handshakes
    # 周期性清理过期握手缓存
    while True:
        # 移除超过5分钟的握手记录
        threshold = time.time() - 300
        used_handshakes = {k:v for k,v in used_handshakes.items() 
                          if v.get('timestamp') > threshold}
...
class EnhancedFakeTLSStreamReader(LayeredStreamReaderBase):
    def __init__(self, upstream):
        self.upstream = upstream
        self.padding_size = random.randint(16, 32) # 随机填充大小
...
async def handle_client(reader_clt, writer_clt):
    ...
    # 增加连接速率限制
    if get_curr_connects_count() > config.MAX_CONNECTIONS:
        await asyncio.sleep(random.uniform(1, 3))
        
    # 增加IP计数跟踪
    ip = writer_clt.get_extra_info('peername')[0]
    if ip_connections.get(ip, 0) > config.MAX_CONNECTIONS_PER_IP:
        return
